...
│   │   ├── wire.ts         # packed report ↔ bytes, no JSON framing; coded form with per-field priors + checksum
│   │   ├── coder.ts        # adaptive binary arithmetic coder (KT counters, integer math, same bytes everywhere)
│   │   ├── labels.ts       # GROUPS: append-only row labels, by section; LIKELY: what each row usually holds
...
- The report lives in the URL fragment (`/#<payload>`; links sent before 15 Sep 2026 read `/#r=<payload>`, `HASH_PREFIX` accepts both), so it never reaches the server and there is no backend. Payload = format flag + base64url: `e` compact as wire bytes (`wire.ts`: tag bytes, no JSON) through the arithmetic coder in `coder.ts`, pure integer JS so every browser writes and reads it without `CompressionStream`. Older links are still read, never written: `b` / `a` (wire bytes deflated / plain), `t` / `u` (compact as JSON in token bytes), `c` / `p` (compact as JSON in UTF-8), `z` / `j` (self-describing JSON). Frozen real links of each generation live in `fixtures.ts`; reading `b`, `t`, `c` and `z` still needs `DecompressionStream` (`needsDecompression` drives the "too old" message).
- Compact links carry values only, a tenth of a JSON link (the Mac fixture: 1729 → 173 characters). Labels, common values and text fragments (swapped for private-use characters, one byte each on the wire) live in `src/lib/link/`, and links already sent index into them: **those lists are append-only**. `wire.ts` caps `COMMON` at 190 entries (the number tag range); it throws at import when the table outgrows it. `DERIVED` holds text other rows determine (screen summary, device pixels, the local time's date and offset): the sender swaps it for one token only when it matches exactly, so a mismatch costs length, never truth. Token slots: 128 for `FRAGMENTS` + `DERIVED` together. A new collector row → a new group at the end of `GROUPS` and, optionally, its usual values in `LIKELY`; until then it travels verbatim in `extras` and the dev console warns. Never edit `FROZEN` in `compact.test.ts` or a frozen link to make a test pass.
- The coder learns as it goes, but a report is too short to learn from, so `wire.ts` primes its models: each row's value is coded under a model seeded with the row's `LIKELY` values (a Yes where a Yes is expected costs about a bit), text characters under a fixed digit-heavy seed. **Every prior is part of the `e` format**: `LIKELY` entries, the character seed and `PRIOR_WEIGHT` are frozen like the lists; a change needs a new flag letter. A coded stream cut short would decode to plausible values, so a seeded 16-bit sum of the wire bytes ends it and a mismatch rejects the link.
